---
kind: period_close_run
period: 2026-06
phase: match
steps: '[{"key": "setup", "label": "Setup", "state": "done"}, {"key": "collect", "label": "Collect", "state": "done", "detail": "statement in"}, {"key": "receipts", "label": "Receipts", "state": "done", "detail": "5 in, 20 photos to look at"}, {"key": "match", "label": "Match", "state": "waiting", "detail": "6 questions for you"}, {"key": "assemble", "label": "Assemble", "state": "pending"}, {"key": "draft", "label": "Draft", "state": "pending"}]'
current_activity: "Waiting on you — 6 need a decision"
status: paused
accountant: "M. Fischer Steuerberatung"
started_at: "2026-06-28T08:55:00Z"
updated_at: "2026-06-28T09:20:00Z"
---

Period-close run state (managed by the period-close workflow).
